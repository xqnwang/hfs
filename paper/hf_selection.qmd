...
where $j^{*}$ denotes the column index of $\hat{\boldsymbol{y}}_{T+h \mid T}^{\prime} \otimes \boldsymbol{S}$ that corresponds to the $j$th column of $\boldsymbol{G}$. Let $\lambda^{k} = 0.0001\lambda^{1}$, the collection of candidate values for $\lambda$ under consideration is $\{\lambda_{0}^{1},...,\lambda_{0}^{k}, 0\}$, where $\lambda^{j} = \lambda^{1}\left(\lambda^{k} / \lambda^{1}\right)^{(j-1) / (k-1)}$ for $j \in [k]$.

**Computation details.** Due to the incorporation of the unbiasedness constraint, we cannot directly use some open-source packages designed for group lasso. Consequently, we employ Gurobi to solve the SOCP problem in @eq-lasso_socp, configuring it with setting OptimalityTol = $0.0001$.

## Series selection method without unbiasedness constraint {#sec-unconstrained}

In-sample based, Empirical group lasso, Elasso

A standard group lasso problem, SOCP formulation is similar to @eq-lasso_socp but no W and constraint

subset and intuitive, try packages...

**Penalty weights and parameter tuning.**

**Computation details.** gglasso but too slow, so gurobi. OptimalityTol = 1e-4

write code and get results for EMinT
