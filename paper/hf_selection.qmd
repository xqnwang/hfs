...
It is challenging to estimate $\bm{W}_h$, especially for $h > 1$. It is common to assume $\bm{W}_h = k_h\bm{W}_1$, $\forall h$, where $k_h > 0$; then the MinT solution of $\bm{G}$ does not change with the forecast horizon, $h$. Hence, we will drop the subscript $h$ for ease of exposition. @tbl-bench lists the most popularly used candidate estimators for $\bm{W}_h$.

```{r}
#| label: tbl-bench
#| tbl-cap: Forecast reconciliation methods for which different estimators of $\bm{W}$ are used.
#| tbl-pos: "!h"
...
::: {.proof}
Let $\bm{X}_{\cdot \mathbb{S}} \in \mathbb{R}^{r \times c}$ denote the matrix $\bm{X}$ with the columns indexed by the set $\mathbb{S}$ set to zeros, and let $\bm{X}_{\mathbb{S}\cdot} \in \mathbb{R}^{r \times c}$ denote the matrix $\bm{X}$ with the rows indexed by $\mathbb{S}$ set to zeros. If $\mathbb{S}$ is the set of indices of nonzero columns in the solution $\hat{\bm{G}}$ to Equation \eqref{eq-op_u}, then the following equations hold: $$
...
$$ This indicates that the number of nonzero columns of $\hat{\bm{G}}$ is no less than $n_b$.

Moreover, based on Equation \eqref{eq-fh}, we have $$
\bm{S}\hat{\bm{G}}_{\cdot\mathbb{S}}\bm{y}_{\mathbb{S}\cdot} = \bm{S}\hat{\bm{G}}_{\cdot\mathbb{S}}\bm{y} = \bm{S}\hat{\bm{G}}_{\cdot\mathbb{S}}\bm{Sb} = \bm{Sb} = \bm{y},
$$
as the solution $\hat{\bm{G}}$ is subject to $\bm{GS}=\bm{I}$. Therefore, the constraint $\bm{GS}=\bm{I}$ ensures that the selected columns of $\hat{\bm{G}}$ correspond to variables capable of "restoring" the hierarchy.
:::

For example, consider the simple hierarchy shown in @fig-hts. Our constrained reconciliation methods might simultaneously zero out the columns of $\bm{G}$ corresponding to series AA and BA, but not to series AA and AB. If the solution to Equation \eqref{eq-op_u} yields a $\hat{\bm{G}}$ with exactly $n_b$ nonzero columns, these correspond to base forecasts from which the full hierarchy can be obtained using nothing but the information embedded in the aggregation constraints. However, if the solution yields a $\hat{\bm{G}}$ with more than $n_b$ nonzero columns, while a coherent forecast can be restored using the the aggregation constraints, it cannot be done uniquely due to the excess of series.
...
In a high-dimensional context with $p \gg n$, it is common to assume that the true regression coefficient (i.e., $\operatorname{vec}(\bm{G})$ in our problem) is sparse. We apply a combination of $L_0$ and $L_2$ regularization to control the nonzero column entries in $\bm{G}$: \begin{align}
...
We present a Big-M based mixed integer programming (MIP) formulation for the problem in Equation \eqref{eq-subset}: \begin{align} \label{eq-subset_mip}
...
#### Intuitive method with $L_0$ regularization {.unnumbered}
...
**Computation details.** Following a setup akin to that in the group best-subset selection, we employ Gurobi to solve Equation \eqref{eq-intuitive_mip} by configuring parameters such as MIPGap = $0.001$ and TimeLimit = $600$ seconds for problems with $p > 1000$.
...
Next, we present the second order cone programming (SOCP) formulation for the group lasso based estimators given by \begin{align}
...
\end{align} Equation \eqref{eq-lasso_socp} includes additional auxiliary variables $c_j \in \mathbb{R}_{\geq 0}$, $j \in [n]$, and second order cone constraints, $c_j = \sqrt{\sum_{i=1}^{n_b} g_{i + (j-1) n_b}^{+2}}$ for $j \in[n]$.
...
::: {#prp-3}
Ignoring the unbiasedness constraint, we define $\lambda^{1}$ as the smallest $\lambda$ value such that all predictors in the group lasso problem have zero coefficients. Then we have $$
