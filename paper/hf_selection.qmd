...
-   $\boldsymbol{G}_h$ is an $n_b \times n$ weighting matrix that maps base forecasts into the bottom level. It combines all base forecasts to form reconciled forecasts for bottom-level series.
-   $\boldsymbol{S}$ is an $n \times n_b$ summing matrix that sums up bottom-level reconciled forecasts to produce coherent forecasts of all levels. It identifies the linear aggregation constraints in a given structure.
...
\midrule
\textbf{OLS} \citep{Hyndman2011-sd} & $\boldsymbol{I}$\\
\textbf{WLSs} \citep{Athanasopoulos2017-jj} & $\operatorname{diag}(\boldsymbol{S} \mathbf{1})$\\
\textbf{WLSv} \citep{Hyndman2016-cz} & $\operatorname{Diag}(\hat{\boldsymbol{W}}_1)$\\
\textbf{MinT} \citep{Wickramasuriya2019-fc} & $\hat{\boldsymbol{W}}_1$\\
\textbf{MinTs} \citep{Wickramasuriya2019-fc} & $\lambda\operatorname{Diag}(\hat{\boldsymbol{W}}_1) + (1-\lambda)\hat{\boldsymbol{W}}_1$\\
\bottomrule
\end{tabular}
