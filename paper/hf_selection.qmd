...
& \text { s.t. } \quad \tilde{\bm{y}}_{T+h \mid T}=\bm{S}\tilde{\bm{b}}_{T+h \mid T},
\end{aligned}
$$ {#eq-mint_op}
...
It is challenging to estimate $\bm{W}_h$, especially for $h > 1$. Assuming that $\bm{W}_h = k_h\bm{W}_1$, $\forall h$, where $k_h > 0$, the MinT solution of $\bm{G}$ does not change with the forecast horizon, $h$. Hence, we will drop the subscript $h$ for the ease of exposition. The most popularly used candidate estimators for $\bm{W}$ in the forecast reconciliation literature are listed in @tbl-bench.
...
\begin{aligned}
& \bm{G}\bm{S} = \hat{\bm{G}}_{\cdot \mathbb{S}}\bm{S}_{\mathbb{S}\cdot} = \bm{I} \text{, and } \\
...
\text { s.t. } \quad & \bar{\bm{G}} = (\bm{S}^{\prime}\bm{A}^{\prime}\bm{W}^{-1}\bm{A}\bm{S})^{-1}\bm{S}^{\prime}\bm{A}^{\prime}\bm{W}^{-1} \\
& \bar{\bm{G}}\bm{S} = \bm{I},
\end{aligned}
$$
