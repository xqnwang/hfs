...
To assess the proposed reconciliation methods with time series selection outlined in @sec-methodology, we carry out two simulations with different designs. Both simulations consider a hierarchy comprising two levels of aggregation, as shown in @fig-hts. The bottom-level series are first generated and then summed to obtain aggregated series at higher levels.
...
rmse_s1 <- readRDS(here::here("paper/results/sim_rmse_s1.rds"))
latex_table(rmse_s1)
```
...
selection_sim <- readRDS(here::here("paper/results/sim_selection.rds"))
mase_sim <- readRDS(here::here("paper/results/sim_series_mase.rds"))

latex_sim_nos_table(selection_sim$out_s1$z, selection_sim$out_s1$n, mase_sim$out_s1, "s1")
```
...
for (scenario in scenarios) {
  rmse_hfs <- readRDS(here::here(paste0("paper/results/sim_rmse_mcb_", scenario, ".rds")))
  if (scenario == "s1"){
    title <- "Scenario A"
  } else if (scenario == "s2"){
    title <- "Scenario B"
...
selection_corr_neg <- readRDS(here::here("paper/results/corr_selection_neg.rds"))
mase_corr_neg <- readRDS(here::here("paper/results/corr_series_mase_neg.rds"))
latex_sim_nos_table(selection_corr_neg$out_s0$z,
                    selection_corr_neg$out_s0$n,
                    mase_corr_neg$out_s0,
                    "corr_neg")
```
...
rmse_s2 <- readRDS(here::here("paper/results/sim_rmse_s2.rds"))
latex_table(rmse_s2)
```
...
rmse_s3 <- readRDS(here::here("paper/results/sim_rmse_s3.rds"))
latex_table(rmse_s3)
```
...
latex_sim_nos_table(selection_sim$out_s2$z, selection_sim$out_s2$n, mase_sim$out_s2, "s2")
```
...
latex_sim_nos_table(selection_sim$out_s3$z, selection_sim$out_s3$n, mase_sim$out_s3, "s3")
```
...
selection_corr_pos <- readRDS(here::here("paper/results/corr_selection_pos.rds"))
mase_corr_pos <- readRDS(here::here("paper/results/corr_series_mase_pos.rds"))
latex_sim_nos_table(selection_corr_pos$out_s0$z,
                    selection_corr_pos$out_s0$n,
                    mase_corr_pos$out_s0,
                    "corr_pos")
```
