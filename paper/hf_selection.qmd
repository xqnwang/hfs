...
where $\mathfrak{g}(\cdot)$ is defined as the exterior penaltyfunction designed to penalize the columns of $\boldsymbol{G}$ towards zero, with $\lambda$ is the corresponding penalty coefficient. The constraint reflects the assumption that the base forecasts and reconciled forecasts are unbiased. When $\lambda = 0$, $\forall h$, the problem reduces to the MinT optimization problem in @eq-mint_op with a closed-form solution given by @eq-mint.

Let

-   The whole structure/framework
...
More on unbiasedness constraint
