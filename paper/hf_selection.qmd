...
#| label: tbl-time
#| results: asis
#| tbl-cap: Computational time (in seconds) for a single hierarchy across the four experiments.
...
One limitation of the proposed methods is their scalability. As the number of series in a hierarchy increases, efficiently solving these problems becomes increasingly challenging, and the exact computation of the estimators remains a significant hurdle. In this study, we employed Gurobi, a widely used commercial solvers, to tackle the NP-hard MIP problems. @tbl-time provides the computational time for solving the reconciliation problem for a single hierarchy in each of our experiments. Note that the reported time considers all candidate hyperparameters. In terms of computational resources we used 20 CPUs and 3 GB of memory per CPU in a high-performance computing (HPC) system. Given these computational demands, we recommend applying the proposed methods primarily to hierarchies of small to moderate size. While there are ongoing efforts to develop MIP-based approaches for solving $L_0$-regularized regression in high-dimensional settings [@Mazumder2022-hx], extending these approaches to accommodate additional constraints remains a challenge, which we leave for future research.
