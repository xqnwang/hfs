...
@tbl-s1-rmse, @tbl-s2-rmse, and @tbl-s3-rmse summarize the results under different scenarios, each reporting the average root mean squared error (RMSE) for each level as well as the whole structure (denoted as *Average*). The *BU* row uses a "bottom-up" approach, aggregating bottom-level base forecasts to form higher level forecasts. The rows labeled *OLS*, *WLS*, *WLSv*, *MinT*, and *MinTs* represent the MinT method using various estimators of $\bm{W}$ (as per @tbl-bench), while the rows *OLS-*, *WLS-*, *WLSv-*, *MinT-*, and *MinTs-* denote the three methods proposed in @sec-constrained, using the corresponding $\bm{W}$ estimators. The *EMinT* row applies the empirical MinT method, while the *Elasso* row uses the empirical group lasso method proposed in @sec-unconstrained. Notably, in each scenario, the largest improvements occur at the level with model misspecification, though this occasionally results in slight performance declines at other levels.
