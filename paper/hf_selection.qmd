...
We now consider to simulate a hierarchical structure with correlated series. We assume the data generating process for the time series at the bottom level follows a first-order vector autoregressive model, i.e., $\text{VAR}(1)$, given by
...
For each time series at the bottom level, we generate a total of $101$ observations, with the last one observation serving as the test set, i.e., $T=100$ and $h=1$. The process is repeated $500$ times for each candidate correlation, $\rho$.

Consider ARMA model, while no constant for series A and BA.
...
Data from Australian Bureau of Statistics: Unemployed persons by Duration of job search, State and Territory, January 1991 onwards available at <https://www.abs.gov.au/statistics/labour/employment-and-unemployment/labour-force-australia-detailed/latest-release>.

Grouped time series

-   Monthly time series

-   Duration of job search (Duration, 6) \* State and territory (STT, 8)

-   $n = 63$ series in total, $n_b = 48$ series at the bottom level

-   Training set: 2010 Jan-2022 Jul, Test set: 2022 Aug-2023 Jul
