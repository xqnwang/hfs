...
%DIF DEL /dev/fd/11               Wed Oct 23 12:42:20 2024
%DIF ADD paper/hf_selection.tex   Wed Oct 23 12:30:29 2024
...
%DIF 84-87c98-102
%DIF < \usepackage{bm}
%DIF < \usepackage{mathptmx}
%DIF < \usepackage[]{natbib}
%DIF < \usepackage[margin = 2.5cm]{geometry}
%DIF -------
\usepackage[textsize=tiny]{todonotes} %DIF > 
\usepackage{bm,mathptmx,amsmath,natbib,microtype,threeparttable,setspace} %DIF > 
\usepackage{colortbl} %DIF > 
\usepackage[onlyrefs]{mathtools} %DIF > 
\usepackage[top=1.5cm, bottom=1.5cm, left=2cm, right=2cm]{geometry} %DIF > 
%DIF -------
\allowdisplaybreaks
\sloppy
%DIF 90-92c105-107
...
%DIF < %\brokenpenalty = 10000
%DIF -------
\clubpenalty = 10000 %DIF > 
\widowpenalty = 10000 %DIF > 
\brokenpenalty = 10000 %DIF > 
%DIF -------
\setlength{\parskip}{0pt}
\setlength{\parindent}{15pt}
%DIF 95-101d110
%DIF < \expandafter\def\expandafter\normalsize\expandafter{%
%DIF <   \normalsize  
%DIF <   \setlength\abovedisplayskip{1ex}
%DIF <   \setlength\belowdisplayskip{1ex}
%DIF <   \setlength\abovedisplayshortskip{1ex}
%DIF <   \setlength\belowdisplayshortskip{1ex}
%DIF < }
%DIF -------
\setlength{\bibsep}{0pt plus 0.3ex}
%DIF 103-104d111
%DIF < \usepackage{microtype}
%DIF < \usepackage[onlyrefs]{mathtools}
%DIF -------
\setlength{\tabcolsep}{0.11cm}
%DIF 106c112-118
%DIF < \usepackage{threeparttable}
%DIF -------
\expandafter\def\expandafter\normalsize\expandafter{% %DIF > 
    \normalsize% %DIF > 
    \setlength\abovedisplayskip{5pt}% %DIF > 
    \setlength\belowdisplayskip{5pt}% %DIF > 
    \setlength\abovedisplayshortskip{3pt}% %DIF > 
    \setlength\belowdisplayshortskip{3pt}% %DIF > 
} %DIF > 
%DIF -------
\makeatletter
...
%DIF 118-133c129-138
%DIF < \captionsetup[figure]{style=italic,format=hang,singlelinecheck=true,font=singlespacing}
%DIF < \captionsetup[table]{style=italic,format=hang,singlelinecheck=true,font=singlespacing}
...
%DIF -------
\captionsetup[figure]{style=italic,singlelinecheck=true,font=singlespacing,format=plain,aboveskip=2pt,belowskip=2pt} %DIF > 
\captionsetup[table]{style=italic,singlelinecheck=true,font=singlespacing,format=plain,aboveskip=2pt,belowskip=2pt} %DIF > 
\setlength{\intextsep}{6pt} % Space above and below in-text figures and tables %DIF > 
\setlength{\textfloatsep}{6pt} % Space between floats and text %DIF > 
 %DIF > 
%% SECTION TITLE %DIF > 
\usepackage{sectsty} %DIF > 
\sectionfont{\vspace{-10pt}} % Space before section title %DIF > 
\subsectionfont{\vspace{-5pt}} % Space before subsection title %DIF > 
\subsubsectionfont{\vspace{-5pt}} % Space before subsubsection title %DIF > 
%DIF -------
\makeatletter
%DIF 135-138d140
%DIF < \makeatother
...
\theoremstyle{plain} %DIF > 
\newtheorem{proposition}{Proposition}[section] %DIF > 
\theoremstyle{remark} %DIF > 
\AtBeginDocument{\renewcommand*{\proofname}{Proof}} %DIF > 
\newtheorem*{remark}{Remark} %DIF > 
\newtheorem*{solution}{Solution} %DIF > 
\newtheorem{refremark}{Remark}[section] %DIF > 
\newtheorem{refsolution}{Solution}[section] %DIF > 
%DIF -------
\makeatother
\makeatletter
%DIF 174a184-185
...
%DIF 198-199c202-203
%DIF <   pdfkeywords={Forecasting, Hierarchical time series, Grouped time
%DIF < series, Linear forecast reconciliation, Integer programming},
%DIF -------
  pdfkeywords={Forecasting, Hierarchical time series, Linear forecast %DIF > 
reconciliation, Variable selection, Integer programming}, %DIF > 
%DIF -------
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
...
\date{ \DIFaddbegin \DIFadd{October 23}\DIFaddend , 2024}
\title{\bf Optimal forecast reconciliation with time series selection}
\author{
Xiaoqian Wang \DIFaddbegin \thanks{Corresponding author. E-mail address:
\href{mailto:xiaoqian.wang@monash.edu}{xiaoqian.wang@monash.edu} (X.
Wang).} \DIFaddend \vspace{0.2em}\\
Department of Econometrics \& Business Statistics, Monash University\DIFaddbegin \DIFadd{,
VIC 3800, Australia }\DIFaddend \vspace{0.2em}\\
...
While forecast reconciliation can enhance overall accuracy in \DIFaddbegin \DIFadd{a
}\DIFaddend hierarchical or grouped  \DIFaddbegin \DIFadd{structure, it can lead to worse forecasts for
certain series, with the greatest gains typically seen in series that
originally have poorly performing }\DIFaddend base forecasts.  \DIFaddbegin \DIFadd{In practical
applications, some }\DIFaddend series in a structure often  \DIFaddbegin \DIFadd{produce }\DIFaddend poor base
forecasts due to model misspecification or low forecastability. To
 \DIFaddbegin \DIFadd{mitigate }\DIFaddend their negative impact, we propose two categories of forecast
reconciliation methods that incorporate \DIFaddbegin \DIFadd{automatic }\DIFaddend time series selection
based on out-of-sample and in-sample information, respectively. These
methods keep ``poor'' base forecasts unused in forming reconciled
forecasts, while adjusting  \DIFaddbegin \DIFadd{the weights assigned }\DIFaddend to the remaining series
accordingly when generating bottom-level reconciled forecasts.
Additionally, our methods ameliorate disparities stemming from varied
 \DIFaddbegin \DIFadd{estimators }\DIFaddend of the base forecast error covariance matrix, alleviating
challenges associated with estimator selection. Empirical evaluations
through two simulation studies and applications using Australian labour
force and domestic tourism data demonstrate  \DIFaddbegin \DIFadd{the potential of the
proposed methods to exclude series with high scaled forecast errors and
show promising results}\DIFaddend .
\end{abstract}

\noindent%
{\it Keywords:} Forecasting, Hierarchical time series,  Linear forecast
reconciliation, \DIFaddbegin \DIFadd{Variable selection, }\DIFaddend Integer programming
\vfill
...
\setstretch{1.5}
 ...
 \DIFaddbegin \renewcommand{\theproposition}{\arabic{proposition}} %DIF >  numbering propositions sequentially and individually by type and not by section
\setcounter{proposition}{0}
\DIFaddend 

\section{Introduction}\label{sec-introduction}

\DIFaddend Forecast reconciliation is a post-processing method that ensures
forecasts of multivariate time series adhere to known linear constraints
\DIFaddbegin \DIFadd{present in the data }\DIFaddend \citep{Hyndman2011-sd}. For example, the sum of
...
\citep{Di_Fonzo2023-vo}.  \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Athanasopoulos2024-sm} }\hskip0pt%DIFAUXCMD
}\DIFaddend provided a
...
base forecasts \citep{Athanasopoulos2017-jj}. In practice,  \DIFaddbegin \DIFadd{hierarchical
...
}\DIFaddend is not uncommon for some series to have poor base forecasts due to
challenges such as model misspecification or low signal-to-noise ratio
(SNR). In such cases, it may be advantageous to exclude  \DIFaddbegin \DIFadd{these
poor-performing }\DIFaddend forecasts when performing reconciliation. This is the
motivation for our proposed methods.

First, we propose \DIFaddbegin \DIFadd{constrained }\DIFaddend forecast reconciliation methods that
incorporate time series selection based on out-of-sample information,
assuming unbiased base forecasts.  \DIFaddbegin \DIFadd{This is formulated }\DIFaddend as an optimization
...
introduce an additional \DIFaddbegin \DIFadd{unconstrained }\DIFaddend reconciliation method with
selection, utilizing in-sample observations and their fitted values.
This  \DIFaddbegin \DIFadd{method leverages }\DIFaddend in-sample reconciliation performance for
selection, \DIFaddbegin \DIFadd{potentially selecting fewer time series }\DIFaddend than the number of
 \DIFaddbegin \DIFadd{bottom-level series, and in extreme cases, resembling a top-down
...
\DIFadd{The methods proposed in this paper incorporate time series selection to
...
addressing }\DIFaddend model misspecification. A remarkable feature of  \DIFaddbegin \DIFadd{our }\DIFaddend methods
is their ability to  \DIFaddbegin \DIFadd{reduce }\DIFaddend disparities arising from using different
 \DIFaddbegin \DIFadd{estimators }\DIFaddend of the base forecast error covariance matrix, thereby
mitigating challenges associated with estimator selection, which is a
prominent concern in  \DIFaddbegin \DIFadd{forecast reconciliation research.
}\DIFaddend 

\DIFaddbegin \DIFadd{Unlike \mbox{%DIFAUXCMD
\citet{Zhang2023-op}}\hskip0pt%DIFAUXCMD
, which keep good forecasts unchanged, we
...
\DIFaddend The remainder of the paper is structured as follows.
...
introduces our proposed methods, \DIFaddbegin \DIFadd{which incorporate }\DIFaddend time series selection
 \DIFaddbegin \DIFadd{to strengthen forecast }\DIFaddend reconciliation, and provides  theoretical
insights. Section~\ref{sec-simulations} and
Section~\ref{sec-applications} show the results from \DIFaddbegin \DIFadd{two }\DIFaddend simulations and
two real-world datasets, respectively. \DIFaddbegin \DIFadd{Section~\ref{sec-discussion}
discusses the practical significance, contributions, limitations, and
potential future research. Finally, }\DIFaddend Section~\ref{sec-conclusion}
 \DIFaddbegin \DIFadd{provides the conclusion. The code and data }\DIFaddend for reproducing the results
...
only the most disaggregated ( bottom-level \DIFaddbegin \DIFadd{) series}\DIFaddend . The full hierarchy
 can be written as \[
...
\(h\)-step-ahead \emph{base forecasts} for all time series in the
 \DIFaddbegin \DIFadd{hierarchy}\DIFaddend , given observations up to and including time \(T\), and
...
(i.e., they won't satisfy the \DIFaddbegin \DIFadd{aggregation }\DIFaddend constraints). Let
\(\tilde{\bm{y}}_{T+h \mid T} \in \mathbb{R}^n\) denote a vector of
\(h\)-step-ahead \emph{reconciled forecasts} given by
\begin{equation} \DIFaddbegin \phantomsection\label{eq-lr}{
\tilde{\bm{y}}_{T+h \mid T} = \bm{S}\bm{G}_h\hat{\bm{y}}_{T+h \mid T},
}\DIFaddend \end{equation} where \(\bm{G}_h\) is an \(n_b \times n\)
 \DIFaddbegin \emph{\DIFadd{weighting matrix}}\DIFadd{.
}\DIFaddend 

 \DIFaddbegin \DIFadd{Forecast reconciliation methods could consider the loss function
\mbox{%DIFAUXCMD
\citep{Ben_Taieb2019-be} }\hskip0pt%DIFAUXCMD
given by
}\begin{equation}\DIFadd{\phantomsection\label{eq-loss}{
\begin{aligned}
...
\end{aligned}
}}\end{equation} \DIFadd{where \(\|\cdot\|_2\) is the \(L_2\) norm. This equation
...
denote the \(h\)-step-ahead \emph{reconciled forecast errors}. \DIFaddbegin \DIFadd{Assuming
...
to zero. \mbox{%DIFAUXCMD
\citet{Wickramasuriya2019-fc} }\hskip0pt%DIFAUXCMD
thus }\DIFaddend formulated the
...
\(h\)-step-ahead covariance matrix of the reconciled forecast errors,
 \DIFaddbegin \DIFadd{leading to }\DIFaddend the unique solution  \DIFaddbegin \DIFadd{given by
}\DIFaddend \begin{equation} \DIFaddbegin \phantomsection\label{eq-mint}{
\bm{G}_h=\left(\bm{S}^{\prime} \bm{W}_h^{-1} \bm{S}\right)^{-1} \bm{S}^{\prime} \bm{W}_h^{-1},
}\DIFaddend \end{equation} where \(\bm{W}_h\) is the positive definite covariance
matrix of the \(h\)-step-ahead base forecast errors.

The MinT problem can be reformulated as a least squares problem with
linear constraints: \begin{equation} \DIFaddbegin \phantomsection\label{eq-mint_op}{
...
}\DIFaddend \end{equation} where
\(\tilde{\bm{b}}_{T+h \mid T} \in \mathbb{R}^{n_b}\) comprises the
\(h\)-step-ahead bottom-level reconciled forecasts, made at time \(T\).
The intuition behind MinT reconciliation is that  \DIFaddbegin \DIFadd{the larger the
estimated variance of the base forecast errors, the larger the range of
adjustments permitted for forecast reconciliation}\DIFaddend .

It is challenging to estimate \(\bm{W}_h\), especially for \(h > 1\). It
is common to assume \(\bm{W}_h = k_h\bm{W}_1\), \(\forall h\), where
\(k_h > 0\); then the MinT solution  \DIFaddbegin \DIFadd{for }\DIFaddend \(\bm{G}\)  \DIFaddbegin \DIFadd{remains unchanged
across different forecast horizons}\DIFaddend , \(h\). Hence, we will drop the
subscript \(h\) for ease of exposition. Table~\ref{tbl-bench} lists the
most popularly used candidate estimators for \(\bm{W}_h\). \DIFaddbegin \DIFadd{In principle,
...
for the optimization process.
 ...
 \DIFaddbeginFL \caption{\label{tbl-bench}\DIFaddFL{Forecast reconciliation methods for which
...
assumption of unbiasedness.  \DIFaddbegin \DIFadd{Their goal was to achieve a tradeoff between
bias and variance by directly minimizing the mean squared reconciled
forecast errors in Equation \eqref{eq-loss}. By }\DIFaddend expanding the training
...
(RERM) problem \DIFaddbegin \DIFadd{: }\DIFaddend \[
...
If \(\hat{\bm{Y}}_{h}^{*\prime}\hat{\bm{Y}}_{h}^{*}\) is not invertible,
 a generalized inverse \DIFaddbegin \DIFadd{can be applied}\DIFaddend . When \(\lambda > 0\), imposing the
\(L_1\) penalty on \(\bm{G}_h\)  \DIFaddbegin \DIFadd{introduces }\DIFaddend sparsity and reduce
estimation variance, albeit at the cost of introducing some bias.

\DIFaddbegin \DIFadd{Relaxing the assumption of unbiasedness of base forecasts,
}\DIFaddend \citet{Wickramasuriya2021-am} proposed an empirical MinT
...
jointly weakly stationary,  \DIFaddbegin \DIFadd{the solution is given by }\DIFaddend \[
...
The difference between EMinT and ERM lies in the data sources. \DIFaddbegin \DIFadd{EMinT is
an ``}\DIFaddend in-sample \DIFaddbegin \DIFadd{'' method in the sense that \(\hat{\bm{Y}}_{h}\) are
predictions in the form of fitted values}\DIFaddend , while ERM  \DIFaddbegin \DIFadd{(and also RERM) is
an ``out-of-sample'' method, with \(\hat{\bm{Y}}_{h}^{*}\) being genuine
forecasts generated on }\DIFaddend a holdout validation set. Both  \DIFaddbegin \DIFadd{EMinT and ERM
}\DIFaddend consider an estimate of \(\bm{G}\) that changes over the forecast
horizon, which is why we keep the subscript \(h\) here.

In practical settings, some series in a  \DIFaddbegin \DIFadd{hierarchy }\DIFaddend could have poor base
...
time series and, consequently, yielding inferior forecasts.  \DIFaddbegin \DIFadd{Moreover,
lower-level }\DIFaddend time series are normally characterized by less apparent
...
perform poorly, as the  weighting matrix \(\bm{G}\)  \DIFaddbegin \DIFadd{assimilates
}\DIFaddend \emph{all} base forecasts and  \DIFaddbegin \DIFadd{maps }\DIFaddend them into bottom-level  forecasts,
...
We therefore propose two  \DIFaddbegin \DIFadd{types of }\DIFaddend forecast reconciliation methods
involving time series selection: constrained \DIFaddbegin \DIFadd{``}\DIFaddend out-of-sample \DIFaddbegin \DIFadd{''
reconciliation, which adheres to the constraint \(\bm{G}\bm{S}=\bm{I}\),
and unconstrained ``}\DIFaddend in-sample \DIFaddbegin \DIFadd{'' reconciliation, which operates without
this constraint}\DIFaddend . These methods aim to address the negative effect of
...
while automatically  \DIFaddbegin \DIFadd{achieving time series selection}\DIFaddend .
Section~\ref{sec-constrained} introduces constrained  \DIFaddbegin \DIFadd{``}\DIFaddend out-of-sample \DIFaddbegin \DIFadd{''
reconciliation methods, formulated based on genuine }\DIFaddend forecasts, while
Section~\ref{sec-unconstrained} presents an unconstrained \DIFaddbegin \DIFadd{``in-sample''
}\DIFaddend reconciliation method, where  \DIFaddbegin \DIFadd{the problem is formulated using }\DIFaddend in-sample
observations and  \DIFaddbegin \DIFadd{predictions in the form of fitted values}\DIFaddend .
...
in Equation  \DIFaddbegin \DIFadd{\eqref{eq-lr}}\DIFaddend . We drop the subscript \(h\) here as we assume
\(\bm{W}\) and \(\bm{G}\) do not vary with the forecast horizon.
 \DIFaddbegin \DIFadd{Recognizing that reconciled forecasts \(\tilde{\bm{y}}\) are simply
linear combinations of base forecasts \(\hat{\bm{y}}\) via
\(\tilde{\bm{y}}=\bm{SG}\hat{\bm{y}}\), setting columns of }\DIFaddend \(\bm{G}\) \DIFaddbegin \DIFadd{to
zero can eliminate heavily misspecified forecasts from the combination}\DIFaddend .
This leads to a generalization of the MinT optimization problem with an
additional penalty term: \begin{equation} \DIFaddbegin \phantomsection\label{eq-op_u}{
...
}\DIFaddend \end{equation} where \(\hat{\bm{y}}:=\hat{\bm{y}}_{T+1 \mid T}\),
\(\mathfrak{g}(\cdot)\) penalizes the columns of \(\bm{G}\) towards
zero, and  \DIFaddbegin \DIFadd{\(\lambda \geq 0\) }\DIFaddend is a penalty parameter. \DIFaddbegin \DIFadd{The methods
developed within this framework are ``out‑of‑sample'' in the sense that
\(\hat{\bm{y}}\) are genuine one-step-ahead forecasts. }\DIFaddend This can be
considered \emph{a grouped variable selection problem}, with each group
corresponding to a column of \(\bm{G}\).  \DIFaddbegin \DIFadd{When \(\lambda = 0\), the
problem reduces to the MinT optimization problem in Equation
\eqref{eq-mint_op} with a closed-form solution.
}

\DIFadd{The constraint }\DIFaddend \(\bm{G}\bm{S}=\bm{I}\)  \DIFaddbegin \DIFadd{guarantees }\DIFaddend that the reconciled
forecasts  \DIFaddbegin \DIFadd{remain }\DIFaddend unbiased if the base forecasts are unbiased. \DIFaddbegin \DIFadd{Under this
assumption and constraint, minimizing the loss function in Equation
\eqref{eq-loss} simplifies }\DIFaddend to the MinT  \DIFaddbegin \DIFadd{problem formulated in Equation
\eqref{eq-mint_op}, which underpins the constrained ``out-of-sample''
reconciliation methods within the framework in equation \eqref{eq-op_u}}\DIFaddend .

 \DIFaddbegin \begin{proposition}[]\protect\hypertarget{prp-1}{}\label{prp-1}

\DIFadd{If the assumption that forecast reconciliation preserves unbiasedness of
the reconciled forecasts is imposed by enforcing \(\bm{GS}=\bm{I}\),
then the number of nonzero column entries of \(\hat{\bm{G}}\) (the solution to Equation \eqref{eq-op_u}) will be no less than \(n_b\).
...
\begin{proof}
\DIFadd{See }\hyperref[appendix-proofs]{Appendix A}, \DIFadd{supplementary materials.
}\end{proof}

\DIFaddend For example,  \DIFaddbegin \DIFadd{for }\DIFaddend the simple hierarchy shown in Figure~\ref{fig-hts},  \DIFaddbegin \DIFadd{the
selected columns of \(\hat{\bm{G}}\) will be at least \(n_b=4\). Our
}\DIFaddend constrained reconciliation methods  \DIFaddbegin \DIFadd{might }\DIFaddend simultaneously zero out \DIFaddbegin \DIFadd{the
}\DIFaddend columns of \(\bm{G}\)  \DIFaddbegin \DIFadd{corresponding to }\DIFaddend series AA and  \DIFaddbegin \DIFadd{BA, but not to
}\DIFaddend series AA and  \DIFaddbegin \DIFadd{AB}\DIFaddend .

 \DIFaddbegin \begin{proposition}[]\protect\hypertarget{prp-2}{}\label{prp-2}

The optimization problem in \DIFadd{Equation \eqref{eq-op_u}} can be reformulated
...
\end{aligned}
}\end{equation}\DIFaddend which is characterized as a high-dimensional problem in
...
\DIFaddend Next, we present three  \DIFaddbegin \DIFadd{constrained ``out-of-sample'' reconciliation
methods}\DIFaddend : (i) group best-subset selection with ridge regularization, (ii)
 \DIFaddbegin \DIFadd{parsimonious }\DIFaddend method with \(L_0\) regularization, and (iii)  group lasso
method. \DIFaddbegin \DIFadd{These methods perform forecast reconciliation with series
...
\(\bm{G}\), \DIFaddbegin \DIFadd{and }\DIFaddend \(\lambda_2 \geq 0\) controls the strength of the ridge
...
The  \DIFaddbegin \DIFadd{best-subsets estimator, derived from an \(L_0\)-regularized least
...
extensively studied; see, for example, \mbox{%DIFAUXCMD
\citet{Greenshtein2004-be}}\hskip0pt%DIFAUXCMD
,
\mbox{%DIFAUXCMD
\citet{Zhang2012-ge}}\hskip0pt%DIFAUXCMD
, and the references therein. However,
\mbox{%DIFAUXCMD
\citet{Mazumder2022-hx} }\hskip0pt%DIFAUXCMD
argued that the vanilla \(L_0\) penalization
...
}\DIFaddend motivated by earlier work on best-subset selection
...
the problem in Equation  \DIFaddbegin \DIFadd{\eqref{eq-subset}:
...
\end{aligned}
}\end{equation}\DIFaddend  where \(\mathcal{M}\) is a Big-M parameter (specified
a-priori) that is sufficiently large that the optimal solution to
Equation  \DIFaddbegin \DIFadd{\eqref{eq-subset_mip}}\DIFaddend , \(\bm{g}^{+*}\), satisfies
\(\max_{j \in [n]}\sum_{i=1}^{n_b} g_{i + (j-1) n_b}^{+} \leqslant \mathcal{M}\)\DIFaddbegin \DIFadd{,
and \([n]=\{1,2,\ldots,n\}\)}\DIFaddend . The binary variable \(z_j=0\) implies that
...
forecasts obtained using Equation  \DIFaddbegin \DIFadd{\eqref{eq-mint} }\DIFaddend with the same
...
\textbf{Computation details.} The MIQP problem in Equation
 \DIFaddbegin \DIFadd{\eqref{eq-subset_mip} }\DIFaddend is NP-hard and computationally intensive.
...
Equation  \DIFaddbegin \DIFadd{\eqref{eq-subset_mip} }\DIFaddend by configuring parameters such as MIPGap
...
the MinT solution in Equation  \DIFaddbegin \DIFadd{\eqref{eq-mint} }\DIFaddend to streamline the
...
either equal to 0 or 1. Taking the MinT solution in Equation
 \DIFaddbegin \DIFadd{\eqref{eq-mint}}\DIFaddend , we have
...
reconciliation method the \emph{ \DIFaddbegin \DIFadd{parsimonious }\DIFaddend method with} \(L_0\)
\emph{regularization} \DIFaddbegin \DIFadd{due to its appeal in reducing the number of
parameters}\DIFaddend . In the results that follow, we label the
\textbf{ \DIFaddbegin \DIFadd{Parsimonious}\DIFaddend } method differently based on various estimators for
\(\bm{W}\), referring to them as \textbf{ \DIFaddbegin \DIFadd{OLS-parsim}\DIFaddend },
\textbf{ \DIFaddbegin \DIFadd{WLSs-parsim}\DIFaddend }, \textbf{ \DIFaddbegin \DIFadd{WLSv-parsim}\DIFaddend }, \textbf{ \DIFaddbegin \DIFadd{MinT-parsim}\DIFaddend }, and
\textbf{ \DIFaddbegin \DIFadd{MinTs-parsim}\DIFaddend }, respectively.

 \DIFaddbegin \DIFadd{In the Parsimonious method, the unknown matrix \(\bm{A}\) is restricted
...
achieving time series }\DIFaddend selection with this optimization problem can be
challenging, \DIFaddbegin \DIFadd{as identifying a solution \(\hat{\bm{A}}\) with some zero
diagonal elements while satisfying both the MinT solution and the
constraint may be difficult. Thus}\DIFaddend , the resulting solution tends to be
...
\begin{equation} \DIFaddbegin \phantomsection\label{eq-parsimonious_mip}{
\begin{aligned}
...
\textbf{Computation details.}  \DIFaddbegin \DIFadd{We }\DIFaddend employ Gurobi to solve Equation
 \DIFaddbegin \DIFadd{\eqref{eq-parsimonious_mip} }\DIFaddend by configuring parameters such as MIPGap =
...
\addcontentsline{toc}{subsubsection}{Group lasso method}
 ...
 \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Yuan2006-mw} }\hskip0pt%DIFAUXCMD
introduced the group lasso method, which extends
...
coefficients. \mbox{%DIFAUXCMD
\citet{Lounici2011-or} }\hskip0pt%DIFAUXCMD
demonstrated that group lasso
...
\mbox{%DIFAUXCMD
\citep[e.g.,][]{Nardi2008-asymptotic}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{When the problem of forecast reconciliation with time series selection
...
size \(n_b\). Thus}\DIFaddend , we consider \emph{a group lasso problem under the
unbiasedness assumption} given by
\begin{equation} \DIFaddbegin \phantomsection\label{eq-lasso}{
\begin{aligned}
...
\text{s.t.} \quad & \left(\bm{S}^{\prime} \otimes \bm{I}_{n_b}\right) \operatorname{vec}(\bm{G})=\operatorname{vec}\left(\bm{I}_{n_b}\right),
\end{aligned}
}\DIFaddend \end{equation} where \(\lambda \geq 0\) is a tuning parameter,
 \DIFaddbegin \DIFadd{\(w_j \geq 0\) }\DIFaddend is the penalty weight assigned in \(\bm{G}_{\cdot j}\) to
...
for the group lasso based estimators: \begin{equation}\phantomsection\label{eq-lasso_socp}{
\begin{aligned}
...
\end{aligned}
}\end{equation}  Equation  \DIFaddbegin \DIFadd{\eqref{eq-lasso_socp} }\DIFaddend includes additional
auxiliary variables \(c_j \in \mathbb{R}_{\geq 0}\), \(j \in [n]\), and
second order cone constraints,
\(c_j = \sqrt{\sum_{i=1}^{n_b} g_{i + (j-1) n_b}^{+2}}\) for
\(j \in[n]\).

Compared to the previous  \DIFaddbegin \DIFadd{methods, group lasso }\DIFaddend is computationally
...
for \(j \in [k]\).
 ...
 \DIFaddbegin \begin{proposition}[]\protect\hypertarget{prp-3}{}\label{prp-3}
\DIFaddend 

 \DIFaddbegin \DIFadd{Relaxing the constraint \(\bm{G_h S}=\bm{I_{n_b}}\)}\DIFaddend , we define
...
\DIFaddend \textbf{Computation details.} Due to the incorporation of the
...
In this section, we relax the unbiasedness  \DIFaddbegin \DIFadd{assumption, }\DIFaddend and introduce a
...
\] where \(\lambda \geq 0\) is a tuning parameter,  \DIFaddbegin \DIFadd{\(w_j \geq 0\) }\DIFaddend is the
...
as \textbf{Elasso} in the results that follow.
 ...
 \DIFaddbegin \DIFadd{Unlike the methods introduced in Section~\ref{sec-constrained}, Elasso
...
\DIFadd{We note that both the ``in-sample'' Elasso and EMinT methods require
...
\DIFadd{Relaxing the unbiasedness assumption }\DIFaddend may result in fewer non-zero column
...
\(\bm{G}_{TD}=[\bm{p} \mid \bm{O}_{n_b \times (n-1)}]\), where
 \DIFaddbegin \DIFadd{\(\bm{p} = (p_1, p_2, \ldots, p_{n_b})\) is a proportionality vector
obtained based on in-sample reconciled forecast errors, and
\(\bm{O}_{n_b \times (n-1)}\) is a matrix of zeros of order
\(n_b \times (n-1)\). Only }\DIFaddend the column corresponding to the top level
(most aggregated level) \DIFaddbegin \DIFadd{of the \(\bm{G}\) matrix }\DIFaddend retains non-zero
values .

We also explored the empirical version of group best-subset selection
with ridge regularization and the  \DIFaddbegin \DIFadd{parsimonious }\DIFaddend method with \(L_0\)
regularization in which we omit the unbiasedness  \DIFaddbegin \DIFadd{assumption}\DIFaddend . It is worth
...
group lasso method for series selection without the unbiasedness
 \DIFaddbegin \DIFadd{assumption}\DIFaddend .
...
\(\{\lambda^{1},\dots,\lambda^{k}, 0\}\), where
 \DIFaddbegin \DIFadd{\(\lambda^{1} = \max_{j=1, \ldots, n}\left\|-\frac{1}{T-T_v}\left(\left(\bm{S} \otimes \hat{\bm{Y}}\right)_{\cdot j*}\right)^{\prime} \operatorname{vec}(\bm{Y})\right\|_2 / w_j\)}\DIFaddend ,
\(\lambda^{k} = 0.0001\lambda^{1}\), and
\(\lambda^{j} = \lambda^{1}\left(\lambda^{k} / \lambda^{1}\right)^{(j-1) / (k-1)}\)
for \(j \in [k]\). Following the derivation in the proof of
 \DIFaddbegin \DIFadd{Proposition~\ref{prp-3}}\DIFaddend , \(\lambda^{1}\) is the smallest \(\lambda\)
...
lasso which aligns with Equation  \DIFaddbegin \DIFadd{\eqref{eq-lasso_socp} }\DIFaddend but omits the
 constraint.
...
levels of aggregation, as shown in Figure~\ref{fig-hts}.  \DIFaddbegin \DIFadd{Bottom-level
series }\DIFaddend are first generated and then summed to obtain \DIFaddbegin \DIFadd{the }\DIFaddend aggregated
...
are randomly sampled from a uniform distribution  \DIFaddbegin \DIFadd{\(U(0.5, 0.7)\)}\DIFaddend , and
...
zero mean and identity covariance matrix.  \DIFaddbegin \DIFadd{Each }\DIFaddend bottom-level series  \DIFaddbegin \DIFadd{has
\(180\) }\DIFaddend observations, with the last \(h = 16\)  forming the test set. The
bottom-level series are  \DIFaddbegin \DIFadd{then aggregated to form higher-level data}\DIFaddend . This
process is repeated \(500\) times.

We use ETS models to generate base forecasts  \DIFaddbegin \DIFadd{for each hierarchy using
}\DIFaddend default settings from the \textbf{forecast} R package
\citep{Hyndman2023-fc}. To introduce model misspecification, we  \DIFaddbegin \DIFadd{degrade
the performance of series A at the middle level by applying a \(1.5\)
multiplier to its }\DIFaddend in-sample \DIFaddbegin \DIFadd{fitted values }\DIFaddend and out-of-sample  forecasts \DIFaddbegin \DIFadd{.
We also repeated the analysis with model misspecification for series }\DIFaddend AA
at the bottom level  and series Total at the top level,  \DIFaddbegin \DIFadd{respectively. The
results for these two scenarios are similar and reported in
}\hyperref[appendix-sim1]{Appendix B}\DIFadd{.
}\DIFaddend 

 \DIFaddbegin \DIFadd{As elaborated following the introduction of each method in
...
\caption{\label{tbl-sim-rmse}\DIFaddFL{Performance of proposed (gray-shaded) and
benchmark methods for simulation Setup 1. The Base row shows average
RMSE of base forecasts, while entries below show RMSE percentage
...
\subcaption{\label{fig-sim-plots-1}\DIFaddFL{MCB test for hierarchy}}

\end{minipage}%DIF > 
%DIF > 
\begin{minipage}{0.04\linewidth}
\DIFaddendFL ~ \DIFaddbeginFL \end{minipage}%DIF > 
%DIF > 
\begin{minipage}{0.48\linewidth}
...
\subcaption{\label{fig-sim-plots-2}\DIFaddFL{Correlation between base forecast
errors}}

\end{minipage}%DIF > 

\caption{\label{fig-sim-plots}\DIFaddFL{MCB test result and correlation matrix
heatmap for simulation Setup 1.}}

\end{figure}%DIF > 

\DIFadd{Table~\ref{tbl-sim-rmse} summarizes the percentage improvement in
}\DIFaddend average root mean squared error (RMSE) for each level  \DIFaddbegin \DIFadd{and the overall
}\DIFaddend structure (denoted as  \DIFaddbegin \DIFadd{Average), relative to the RMSE of the }\DIFaddend base
forecasts. The  \DIFaddbegin \DIFadd{BU row shows results for the }\DIFaddend ``bottom-up'' approach,
 \DIFaddbegin \DIFadd{where base forecasts at the bottom level are aggregated to generate
...
statistically different if the intervals of two methods do not overlap.
 ...
 \DIFaddbegin \DIFadd{We }\DIFaddend find that the BU  \DIFaddbegin \DIFadd{method }\DIFaddend performs the best  \DIFaddbegin \DIFadd{overall, which is
unsurprising given that the aggregated series A is deteriorated in this
setup, and the BU method does not use its information. }\DIFaddend WLSv, MinT, and
MinTs  \DIFaddbegin \DIFadd{also perform well, with no significant difference from BU, as they
account for the }\DIFaddend in-sample covariance of base forecast errors, allowing
for  \DIFaddbegin \DIFadd{larger }\DIFaddend adjustments in reconciliation for  forecasts with higher  error
variance.  However, OLS and WLSs significantly underperform \DIFaddbegin \DIFadd{the }\DIFaddend other
benchmark methods. \DIFaddbegin \DIFadd{Our proposed methods show significant improvements
}\DIFaddend when using OLS and WLSs estimators of \(\bm{W}\) , which do not  \DIFaddbegin \DIFadd{consider
}\DIFaddend in-sample covariance. \DIFaddbegin \DIFadd{A key advantage of our }\DIFaddend forecast reconciliation
methods with selection is their ability to reduce  \DIFaddbegin \DIFadd{differences }\DIFaddend introduced
by using different  \DIFaddbegin \DIFadd{estimators }\DIFaddend of \(\bm{W}\), thereby mitigating the risk
of estimator selection.  \DIFaddbegin \DIFadd{We }\DIFaddend can align the forecast accuracy achieved
using different estimators,  \DIFaddbegin \DIFadd{making }\DIFaddend them approach the best results we can
obtain.  \DIFaddbegin \DIFadd{By relaxing }\DIFaddend the unbiasedness assumption, Elasso performs
similarly to EMinT overall, \DIFaddbegin \DIFadd{with slight improvements at aggregated
levels}\DIFaddend , which is typically the  \DIFaddbegin \DIFadd{primary concern for }\DIFaddend practitioners.

 \begin{table}

 \DIFaddbeginFL \caption{\label{tbl-sim-selection}\DIFaddFL{Proportion of time series being
selected using proposed reconciliation methods for simulation Setup 1.
RMSSE values of base forecasts for each series are in parentheses. The
last column displays a stacked barplot of the total selected series from
500 hierarchies, with darker sub-bars indicating higher counts.}}
\DIFaddendFL 
...
In addition, we report the proportion of time series being selected  \DIFaddbegin \DIFadd{by
}\DIFaddend our proposed methods  \DIFaddbegin \DIFadd{across \(500\) }\DIFaddend simulation instances, \DIFaddbegin \DIFadd{along with the
average root mean squared scaled error (RMSSE) for base forecasts of
each series, }\DIFaddend as shown in Table~ \DIFaddbegin \DIFadd{\ref{tbl-sim-selection}. Our }\DIFaddend methods
select fewer time series, while enhancing forecast accuracy compared to
benchmarks. Subset methods, in particular,  \DIFaddbegin \DIFadd{select }\DIFaddend fewer time series  \DIFaddbegin \DIFadd{than
Parsimonious and Lasso}\DIFaddend , which aligns with our expectations that the
 \DIFaddbegin \DIFadd{Parsimonious }\DIFaddend and Lasso methods tend to produce dense estimates.  \DIFaddbegin \DIFadd{Notably,
the misspecified series A shows higher RMSSE and is selected least
frequently, except in cases where all bottom-level series are retained
so series A leads to only slight deterioration. Series Total and B are
also selected less often maybe due to the high correlation of their
forecast errors with other series, as shown in
Figure~\ref{fig-sim-plots-2}}\DIFaddend .
...
exclude them from reconciliation in this setup.
 ...
 \DIFaddbegin \DIFadd{Similarly to Section~\ref{sec-sim1}, we experimented with using the
...
\DIFaddend \begin{table}
 ...
 \DIFaddbeginFL \caption{\label{tbl-corr-rmse}\DIFaddFL{Performance of proposed (gray-shaded) and
benchmark methods for simulation Setup 2. The Base row shows average
RMSE of base forecasts, while entries below show RMSE percentage
decrease (negative) or increase (positive) for reconciliation methods.
Blue entries highlight the best-performing methods; bold entries
indicate proposed methods outperforming bechmarks.}}
...
\subcaption{\label{fig-corr-plots-1}\DIFaddFL{MCB test for hierarchy}}

\end{minipage}%DIF > 
%DIF > 
\begin{minipage}{0.04\linewidth}
\DIFaddFL{~}\end{minipage}%DIF > 
%DIF > 
\begin{minipage}{0.48\linewidth}
...
\subcaption{\label{fig-corr-plots-2}\DIFaddFL{Correlation between base forecast
errors}}

\end{minipage}%DIF > 

\caption{\label{fig-corr-plots}\DIFaddFL{MCB test result and correlation matrix
heatmap for simulation Setup 2, with error correlation being -0.8.}}

\end{figure}%DIF > 

\DIFadd{The results across various error correlations are presented in
}\DIFaddend Table~\ref{tbl-corr-rmse}, \DIFaddbegin \DIFadd{with the MCB test for a correlation of
...
thus we report them in }\hyperref[appendix-sim2]{Appendix C}\DIFaddend . The results
show that  our proposed methods  \DIFaddbegin \DIFadd{slightly outperform or are comparable }\DIFaddend to
their respective  \DIFaddbegin \DIFadd{benchmarks }\DIFaddend at all levels  \DIFaddbegin \DIFadd{when using OLS, WLSs, and WLSv
estimators. However, only the OLS-lasso method significantly outperforms
the OLS benchmark. It is important to note the }\DIFaddend challenge of identifying
...
well, \DIFaddbegin \DIFadd{and our proposed methods provide similar results}\DIFaddend . This is
...
high estimated error variance. Elasso  \DIFaddbegin \DIFadd{performs the best across the whole
hierarchy, though not significantly better than EMinT }\DIFaddend due to the
 \DIFaddbegin \DIFadd{identification challenge}\DIFaddend . We have also considered alternative error
...
results follow a similar pattern and are available upon request.
 ...
 Table~\ref{tbl-corr-selection-neg}  \DIFaddbegin \DIFadd{presents }\DIFaddend the proportion of time
series  selected using our  \DIFaddbegin \DIFadd{methods, with an error correlation of
...
}\hyperref[appendix-sim2]{Appendix C}. \DIFadd{Despite the challenges in
identifying poor-performing series in this setup,
}\DIFaddend Table~\ref{tbl-corr-selection-neg} shows that  \DIFaddbegin \DIFadd{Subset and Parsimonious
methods, using }\DIFaddend OLS, WLSs, and WLSv estimators,  are able to exclude the
series Total, A, and BA \DIFaddbegin \DIFadd{(which have higher RMSSE) }\DIFaddend in some instances \DIFaddbegin \DIFadd{.
...
in Figure~\ref{fig-corr-plots-2}}\DIFaddend . Subset methods outperform  \DIFaddbegin \DIFadd{Parsimonious
}\DIFaddend methods in selection. Lasso methods typically select all bottom-level
series, given their tendency to yield dense estimates, as discussed in
Section~\ref{sec-constrained}.  \DIFaddbegin \DIFadd{When addressing higher correlation
values, as shown in }\hyperref[appendix-sim2]{Appendix C}, \DIFadd{our methods
maintain the ability to exclude }\DIFaddend series that should be omitted in
reconciliation. \DIFaddbegin \DIFadd{However, their efficacy diminishes in the presence of
positive correlation, particularly in removing the bottom-level series
BA. }\DIFaddend Hence, our methods are  \DIFaddbegin \DIFadd{especially preferable }\DIFaddend when the error
correlation within the structure is negative.

 \begin{table}

 \DIFaddbeginFL \caption{\label{tbl-corr-selection-neg}\DIFaddFL{Proportion of time series
selected using proposed reconciliation methods for simulation Setup 2,
with error correlation being -0.8. RMSSE values of base forecasts for
each series are in parentheses. The last column displays a stacked
...
persons in Australia from January 2010 to July 2023\footnote{The Labour
 ...
 \DIFaddbegin \DIFadd{We address }\DIFaddend the few missing observations  \DIFaddbegin \DIFadd{using }\DIFaddend linear interpolation.
...
NSW , \DIFaddbegin \DIFadd{VIC, QLD, SA, WA, TAS, NT, }\DIFaddend and ACT . So the final grouped hierarchy
...
representing the top-level series in the  \DIFaddbegin \DIFadd{hierarchy}\DIFaddend . The monthly series
...
NSW and QLD, while the seasonal pattern in VIC differs.  \DIFaddbegin \DIFadd{Compared to the
STT level}\DIFaddend , the seasonal patterns in the Duration-level series are more
...
affect the accuracy of forecasts across all time series.
 ...
 \DIFaddbeginFL \caption{\label{tbl-labour-rmse-avg}\DIFaddFL{Performance of proposed
...
The  \DIFaddbegin \DIFadd{MCB test and error correlation calculations are not performed due to
the limited data length. Results of MinT and the respective proposed
methods are excluded due to poor performance, likely caused by }\DIFaddend the poor
sample covariance  \DIFaddbegin \DIFadd{estimate given the small sample size relative to }\DIFaddend the
number of series in the structure. Subset methods using different
estimators of  \DIFaddbegin \DIFadd{\(\bm{W}\) }\DIFaddend generally improve forecast accuracy over their
 \DIFaddbegin \DIFadd{benchmarks, particularly at }\DIFaddend aggregation levels, which are  of paramount
concern to practitioners. The  \DIFaddbegin \DIFadd{exception is }\DIFaddend WLSs-subset, \DIFaddbegin \DIFadd{which shows
slight deterioration, though still improving }\DIFaddend top-level forecasts.
Moreover, the  \DIFaddbegin \DIFadd{Parsimonious }\DIFaddend and Lasso methods almost always yield results
identical to  \DIFaddbegin \DIFadd{their benchmarks}\DIFaddend , because they tend to provide dense
estimates, and ETS models typically  \DIFaddbegin \DIFadd{avoid }\DIFaddend extremely poor forecasts.
 \DIFaddbegin \DIFadd{However, OLS-parsim shows deterioration at all levels except }\DIFaddend the top
level . When we drop the unbiasedness assumption, EMinT  \DIFaddbegin \DIFadd{performs the
worst }\DIFaddend across all levels, \DIFaddbegin \DIFadd{as it assumes joint weak stationarity among
series}\DIFaddend , which is evidently not the case  \DIFaddbegin \DIFadd{here. Elasso ranks the best
overall and significantly outperforms }\DIFaddend EMinT, with the most accurate
coherent forecasts observed at the top level and STT level.

 \DIFaddbegin \DIFadd{Results }\DIFaddend based on the final test set spanning from August 2022 to July
2023  \DIFaddbegin \DIFadd{are detailed in }\hyperref[appendix-labour]{Appendix D}, \DIFadd{allowing
}\DIFaddend for model training  \DIFaddbegin \DIFadd{with more data and analysis of }\DIFaddend post-COVID  \DIFaddbegin \DIFadd{patterns.
Table~\ref{tbl-labour-rmse} shows that the results are qualitatively
similar to those }\DIFaddend in Table~\ref{tbl-labour-rmse-avg} \DIFaddbegin \DIFadd{.
}\DIFaddend Table~\ref{tbl-labour-info} presents the number of series selected at
each level, \DIFaddbegin \DIFadd{their average RMSSE values, and the identified optimal
tuning parameters. Only }\DIFaddend Subset and Elasso methods \DIFaddbegin \DIFadd{are presented}\DIFaddend , as they
 \DIFaddbegin \DIFadd{differ from benchmark }\DIFaddend results. The scale variation in optimal parameters
 \DIFaddbegin \DIFadd{across }\DIFaddend methods is due to \DIFaddbegin \DIFadd{differences in }\DIFaddend objective function scales.
 \DIFaddbegin \DIFadd{Notably, }\DIFaddend all Subset methods exclude some series, \DIFaddbegin \DIFadd{and Elasso, using }\DIFaddend only
\(11\) series, \DIFaddbegin \DIFadd{performs best overall when assessed based on average
results across the entire hierarchy and forecast horizon. With the
exception }\DIFaddend of the series \DIFaddbegin \DIFadd{selected }\DIFaddend at the STT level  \DIFaddbegin \DIFadd{by WLSs-subset, and
the }\DIFaddend series at the Duration level  \DIFaddbegin \DIFadd{by Elasso, the time series chosen by
...
Base row in the table)}\DIFaddend .
...
\DIFaddendFL 
 ...
 \DIFaddbeginFL \caption{\label{fig-tourism-data}\DIFaddFL{Domestic tourism flows from January
...
across  \DIFaddbegin \DIFadd{both national and states levels, though patterns vary across
series. Significant }\DIFaddend growth began around 2010 for the national  flow and
some states such as NSW, VIC, QLD, and WA, \DIFaddbegin \DIFadd{while flows }\DIFaddend for SA, TAS, and
NT \DIFaddbegin \DIFadd{are relatively flat}\DIFaddend . Moreover,  \DIFaddbegin \DIFadd{there is a notable decline }\DIFaddend in tourism
...
\caption{\label{tbl-tourism-rmse-avg}\DIFaddFL{Performance of proposed
...
\DIFaddend Table~\ref{tbl-tourism-rmse-avg} reports  \DIFaddbegin \DIFadd{forecast performance results}\DIFaddend .
Similar to Section~\ref{sec-labour}, MinT and the respective proposed
methods are  \DIFaddbegin \DIFadd{excluded due to poor performance, and the MCB test and error
correlation calculations are omitted due to limited data}\DIFaddend . The results
show that  \DIFaddbegin \DIFadd{OLS }\DIFaddend outperforms other benchmarks, \DIFaddbegin \DIFadd{despite }\DIFaddend WLSv and MinTs
 \DIFaddbegin \DIFadd{accounting for }\DIFaddend in-sample covariance of base forecast errors \DIFaddbegin \DIFadd{,
highlighting its effectiveness. }\DIFaddend Overall, the Subset methods outperform
 \DIFaddbegin \DIFadd{benchmarks}\DIFaddend , especially for aggregation levels . The only exception is
 OLS-subset , which slightly reduces overall accuracy while improving
top-level forecasts.  \DIFaddbegin \DIFadd{Parsimonious }\DIFaddend and Lasso methods produce results
almost identical to  \DIFaddbegin \DIFadd{their }\DIFaddend benchmarks, which is not surprising as ETS
...
we relax the unbiasedness  \DIFaddbegin \DIFadd{assumption}\DIFaddend , EMinT consistently performs  worst
across all levels due to the  lack of joint weak stationarity among the
series in the hierarchy.  \DIFaddbegin \DIFadd{Elasso, however, shows large }\DIFaddend improvement
compared to EMinT, and  \DIFaddbegin \DIFadd{ranks best across most }\DIFaddend levels except the bottom
level.

 \DIFaddbegin \DIFadd{We }\DIFaddend present the results based on the last  \DIFaddbegin \DIFadd{test set }\DIFaddend from January 2017 to
December  \DIFaddbegin \DIFadd{2017 in }\hyperref[appendix-tourism]{Appendix E}\DIFadd{.
Table~\ref{tbl-tourism-rmse} shows the percentage improvement in RMSEs
for various reconciliation methods, and }\DIFaddend Figure~\ref{fig-tourism-rmse}
 \DIFaddbegin \DIFadd{displays the reconciliation errors across \(111\) series. These results
are consistent with }\DIFaddend the average results  \DIFaddbegin \DIFadd{mentioned earlier}\DIFaddend , indicating
relatively high-quality forecasts from Subset and Elasso methods.  \DIFaddbegin \DIFadd{In
addition}\DIFaddend , Table~\ref{tbl-tourism-info} summarizes the number of series
selected  \DIFaddbegin \DIFadd{at each level, their average RMSSE values, and }\DIFaddend the optimal
tuning  \DIFaddbegin \DIFadd{parameters}\DIFaddend . Here we focus on the Subset and Elasso methods since
they  \DIFaddbegin \DIFadd{show improved forecast results in this application. }\DIFaddend We observe that
 OLS-subset and WLSs-subset  exclude some series at the State and Zone
levels, \DIFaddbegin \DIFadd{yielding lower RMSSE values for the selected series at each
...
exclude series with high scaled forecast errors. }\DIFaddend In contrast, the WLSv
and MinTs \DIFaddbegin \DIFadd{subset }\DIFaddend methods retain all series. \DIFaddbegin \DIFadd{Nonetheless, the inclusion
...
\DIFadd{While our proposed methods opt to leave ``poor'' base forecasts unused
in forming reconciled forecasts, the approach by \mbox{%DIFAUXCMD
\citet{Zhang2023-op} }\hskip0pt%DIFAUXCMD
is
...
\mbox{%DIFAUXCMD
\citet{Zhang2023-op} }\hskip0pt%DIFAUXCMD
in four main ways. First, our proposed methods keep
the poor-performing base forecasts unused, ensuring these do not
influence other nodes, whereas the approach by \mbox{%DIFAUXCMD
\citet{Zhang2023-op}
}\hskip0pt%DIFAUXCMD
...
reconciliation, while \mbox{%DIFAUXCMD
\citet{Zhang2023-op} }\hskip0pt%DIFAUXCMD
use base forecasts of all
...
unbiasedness assumption and introduce an unconstrained ``}\DIFaddend in-sample \DIFaddbegin \DIFadd{''
reconciliation method that leverages }\DIFaddend in-sample  \DIFaddbegin \DIFadd{information for
selection.
}\DIFaddend 

 \DIFaddbegin \DIFadd{The proposed methods offer four key benefits to hierarchical forecasting
...
\caption{\label{tbl-time}\DIFaddFL{Computational time (in seconds) for a single
hierarchy across the four experiments, considering all candidate
hyperparameters and rolling test sets.}}
...
\end{tabular}
\begin{tablenotes}[para]
\item \footnotesize{Note: NOCH refers to the number of candidate hyperparameters considered. A dash (-) denotes that the method is not applicable to the experiment.}
\end{tablenotes}
\end{threeparttable}
\endgroup{}
...
\DIFadd{One limitation of the proposed methods is their scalability. As the
number of series in a hierarchy increases, solving these problems
efficiently becomes increasingly challenging, and the exact computation
of the estimates remains a significant hurdle. In this study, we
employed Gurobi, a widely used commercial solvers, to tackle the NP-hard
MIP problems. Table~\ref{tbl-time} provides the computational time for
solving the reconciliation problem for a single hierarchy in each of our
experiments, considering all candidate hyperparameters and rolling test
sets. In terms of computational resources we used 20 CPUs and 3 GB of
memory per CPU in a high-performance computing system. Given these
computational demands, we recommend applying the proposed methods
primarily to hierarchies of small to moderate size. While there are
ongoing efforts to develop MIP-based approaches for solving
\(L_0\)-regularized regression in high-dimensional settings
\mbox{%DIFAUXCMD
\citep{Mazumder2022-hx}}\hskip0pt%DIFAUXCMD
, extending these approaches to accommodate
additional constraints remains a challenge, which we leave for future
research.
}

\DIFadd{Other promising directions for further investigation are also worth
...
optimization problem }\[
\DIFadd{\min_{\bm{G}} \quad \frac{1}{2 T} \operatorname{Tr}\left( \left(\bm{Y}-\hat{\bm{Y}} \bm{G}^{\prime} \bm{S}^{\prime}\right)\bm{V}^{-1}\left(\bm{Y}-\hat{\bm{Y}} \bm{G}^{\prime} \bm{S}^{\prime}\right)^{\prime}\right) + \lambda \sum_{j=1}^n w_j \left\|\bm{G}_{\cdot j}\right\|_2,
}\] \DIFadd{where \(\operatorname{Tr}(\cdot)\) denotes the trace of a matrix. In
...
\DIFadd{Additionally, \mbox{%DIFAUXCMD
\citet{Panagiotelis2021-mf} }\hskip0pt%DIFAUXCMD
highlighted the critical need
...
\DIFadd{\min_{\bm{G}, \bm{d}} \quad \frac{1}{2 T} \left\|\bm{Y}-\left(\bm{D} + \hat{\bm{Y}} \bm{G}^{\prime}\right)\bm{S}^{\prime}\right\|_F^2 + \lambda \sum_{j=1}^n w_j \left\|\bm{G}_{\cdot j}\right\|_2,
}\] \DIFadd{where \(\bm{D} \in \mathbb{R}^{T \times n_b}\) is a shift parameter,
with each row equal to an \(n_b\)-dimensional \(\bm{d}\). This vector
\(\bm{d}\) can be trained alongside \(\bm{G}\) and act as a bias
correction.
}

\DIFadd{Finally, the concept of bi-level variable selection can be applied to
...
\mbox{%DIFAUXCMD
\citep{Simon2013-sp}}\hskip0pt%DIFAUXCMD
, hierarchical lasso \mbox{%DIFAUXCMD
\citep{Zhou2010-vs}}\hskip0pt%DIFAUXCMD
, and group
bridge \mbox{%DIFAUXCMD
\citep{Huang2009-vs}}\hskip0pt%DIFAUXCMD
. By treating each column of \(\bm{G}\) as a
...
this case, within-group sparsity can be achieved by including an
additional lasso penalty to shrink individual elements, as suggested in
\mbox{%DIFAUXCMD
\citet{Simon2013-sp}}\hskip0pt%DIFAUXCMD
. This bi-level selection mechanism could
...
\DIFadd{In the }\DIFaddend existing forecast reconciliation literature,  \DIFaddbegin \DIFadd{base forecasts are
mapped }\DIFaddend into bottom-level disaggregated forecasts, which are then summed
to yield coherent forecasts for the entire structure. The mapping step
can be conceptually regarded as a forecast combination.  \DIFaddbegin \DIFadd{However, poor
performance in some base forecasts can diminish }\DIFaddend overall reconciliation
effectiveness.  \DIFaddbegin \DIFadd{To address this issue, we incorporate time series
selection to enhance forecast reconciliation, }\DIFaddend while ensuring coherent
...
Under the assumption of unbiased base forecasts}\DIFaddend , we developed three
\DIFaddbegin \DIFadd{constrained ``out-of-sample'' }\DIFaddend reconciliation methods with selection
mechanisms . These methods  \DIFaddbegin \DIFadd{use various penalty functions to shrink }\DIFaddend the
columns of the weighting matrix  \(\bm{G}\)  towards zero. \DIFaddbegin \DIFadd{We proved that
the selected time series can effectively reconstruct the whole
hierarchy. }\DIFaddend Additionally, we relaxed the unbiasedness  \DIFaddbegin \DIFadd{assumption by
introducing the unconstrained Elasso method, which selects time }\DIFaddend series
based on in-sample observations and fitted values. \DIFaddbegin \DIFadd{Elasso may use fewer
...
methods . When model misspecification was introduced  \DIFaddbegin \DIFadd{in some series
within }\DIFaddend the hierarchy, our  \DIFaddbegin \DIFadd{methods effectively excluded these
``problematic'' series, ensuring good quality coherent forecasts}\DIFaddend . In
both empirical applications, where no apparent model misspecification
was present, \DIFaddbegin \DIFadd{the }\DIFaddend Subset and Elasso methods  \DIFaddbegin \DIFadd{demonstrated promising
results. In contrast, Parsimonious and Lasso generally produced results
similar to their }\DIFaddend benchmarks, as they tend to  \DIFaddbegin \DIFadd{deliver }\DIFaddend dense estimates.

 \DIFaddbegin \DIFadd{While this study provides an important advancement in hierarchical
...
\textbf{Appendix:}  \DIFaddbegin \DIFadd{Summary of proofs, and additional }\DIFaddend results obtained in
Section~\ref{sec-simulations} and Section~\ref{sec-applications}.

\section*{Acknowledgments}\label{acknowledgments}
\addcontentsline{toc}{section}{Acknowledgments}

\DIFaddbegin \DIFadd{Xiaoqian Wang and }\DIFaddend Rob J Hyndman  \DIFaddbegin \DIFadd{are members of }\DIFaddend the Australian Research
...
\newpage
\appendix
 \DIFaddbegin \renewcommand{\thesubsection}{\Alph{subsection}}
...
\setcounter{table}{0}
 ...
 \DIFaddbegin \section*{\DIFadd{Appendix}}\label{appendix}
\DIFaddend \addcontentsline{toc}{section}{Appendix}
...
\DIFadd{Let \(\bm{X}_{\cdot \mathbb{S}} \in \mathbb{R}^{r \times |\mathbb{S}|}\)
...
\DIFadd{\hat{\bm{G}}\bm{S} = \hat{\bm{G}}_{\cdot \mathbb{S}}\bm{S}_{\mathbb{S}\cdot} = \bm{I}_{n_b},
\qquad\text{and}\qquad
...
}\] \DIFadd{This indicates that the number of nonzero columns of \(\hat{\bm{G}}\)
should be no less than \(n_b\), i.e., \(|\mathbb{S}| \geq n_b\).
}

\DIFadd{Moreover, we have
...
\mbox{%DIFAUXCMD
\citet{Zhang2023-op}}\hskip0pt%DIFAUXCMD
, \(\bm{y}_{\mathbb{S}\cdot}\) is valid for
...
\subsubsection{\DIFadd{Proof of Proposition 2}}\label{proof-of-proposition-2}

\begin{proof}
\DIFadd{We have}\vspace*{-0.4cm}\enlargethispage{0.4cm} \[
...
\end{aligned}
}\] \DIFadd{Substituting these into Equation \eqref{eq-op_u}, the previous
...
\subsubsection{\DIFadd{Proof of Proposition 3}}\label{proof-of-proposition-3}

\begin{proof}
\DIFadd{Denote \(\bm{\beta} = \operatorname{vec}(\bm{G})\), and the first term
...
\DIFadd{The section provides additional results obtained in
Section~\ref{sec-sim1}.
}

\begin{table}[!h]

\caption{\label{tbl-sim-rmse-total}\DIFaddFL{Performance of proposed (gray-shaded)
and benchmark methods for simulation Setup 1, with model
...
\DIFaddendFL 
 ...
 \DIFaddbeginFL \begin{minipage}{0.48\linewidth}
...
\subcaption{\label{fig-sim-plots-total-1}\DIFaddFL{MCB test}}
...
\subcaption{\label{fig-sim-plots-total-2}\DIFaddFL{Correlation between base
forecast errors}}

\end{minipage}%DIF > 

\caption{\label{fig-sim-plots-total}\DIFaddFL{MCB test result and correlation
matrix heatmap for simulation Setup 1, with model misspecification in
series Total.}}
...
\caption{\label{tbl-sim-selection-total}\DIFaddFL{Proportion of time series being
selected using proposed reconciliation methods for simulation Setup 1,
with model misspecification in series Total. RMSSE values of base
forecasts for each series are in parentheses. The last column displays a
stacked barplot of the total selected series from 500 hierarchies, with
darker sub-bars indicating higher counts.}}
...
\DIFaddendFL 
 ...
 \DIFaddbeginFL \caption{\label{tbl-sim-rmse-AA}\DIFaddFL{Performance of proposed (gray-shaded)
and benchmark methods for simulation Setup 1, with model
...
\subcaption{\label{fig-sim-plots-AA-1}\DIFaddFL{MCB test}}
...
\subcaption{\label{fig-sim-plots-AA-2}\DIFaddFL{Correlation between base forecast
errors}}

\end{minipage}%DIF > 

\caption{\label{fig-sim-plots-AA}\DIFaddFL{MCB test result and correlation matrix
heatmap for simulation Setup 1, with model misspecification in series
AA.}}
...
\caption{\label{tbl-sim-selection-AA}\DIFaddFL{Proportion of time series being
selected using proposed reconciliation methods for simulation Setup 1,
with model misspecification in series AA. RMSSE values of base forecasts
for each series are in parentheses. The last column displays a stacked
barplot of the total selected series from 500 hierarchies, with darker
sub-bars indicating higher counts.}}
...
\subsection{Results from simulation Setup 2}\label{appendix-sim2}
 ...
 \DIFaddbegin \DIFadd{The section provides additional results obtained in
Section~\ref{sec-sim2}.
}

\begin{table}[!h]

\caption{\label{tbl-corr-selection-p3}\DIFaddFL{Proportion of time series selected
using proposed reconciliation methods for simulation Setup 2, with error
correlation being -0.4. RMSSE values of base forecasts for each series
are in parentheses. The last column displays a stacked barplot of the
total selected series from 500 hierarchies, with darker sub-bars
...
\DIFaddendFL 
 ...
 \DIFaddbeginFL \caption{\label{tbl-corr-selection-p5}\DIFaddFL{Proportion of time series selected
using proposed reconciliation methods for simulation Setup 2, with error
correlation being 0. RMSSE values of base forecasts for each series are
in parentheses. The last column displays a stacked barplot of the total
selected series from 500 hierarchies, with darker sub-bars indicating
higher counts.}}
...
\DIFaddendFL 
 ...
 \DIFaddbeginFL \caption{\label{tbl-corr-selection-p7}\DIFaddFL{Proportion of time series selected
using proposed reconciliation methods for simulation Setup 2, with error
correlation being 0.4. RMSSE values of base forecasts for each series
are in parentheses. The last column displays a stacked barplot of the
total selected series from 500 hierarchies, with darker sub-bars
...
\DIFaddendFL 
 ...
 \DIFaddbeginFL \caption{\label{tbl-corr-selection-p9}\DIFaddFL{Proportion of time series selected
using proposed reconciliation methods for simulation Setup 2, with error
correlation being 0.8. RMSSE values of base forecasts for each series
are in parentheses. The last column displays a stacked barplot of the
total selected series from 500 hierarchies, with darker sub-bars
indicating higher counts.}}
...
\DIFaddendFL 
 ...
 \DIFaddbeginFL \begin{minipage}{\linewidth}
\DIFaddendFL 
...
\subcaption{\label{fig-corr-plots-others-1}\DIFaddFL{MCB test}}
...
\subcaption{\label{fig-corr-plots-others-2}\DIFaddFL{Correlation between base
forecast errors}}

\end{minipage}%DIF > 

\caption{\label{fig-corr-plots-others}\DIFaddFL{MCB test result and correlation
matrix heatmap for simulation Setup 2, with error correlation being
-0.4, 0, 0.4, 0.8, respectively.}}
...
\caption{\label{tbl-labour-rmse}\DIFaddFL{Out-of-sample forecast results on a
...
\caption{\label{tbl-labour-info}\DIFaddFL{Number of time series selected by
...
2023). The Base row shows the original number of series in the
structure. The numbers in parentheses show RMSSE values of base
forecasts for selected series across different levels.}}
...
\caption{\label{tbl-tourism-rmse}\DIFaddFL{Out-of-sample forecast results on a
...
\caption{\label{fig-tourism-rmse}\DIFaddFL{Average out-of-sample forecasting
...
\caption{\label{tbl-tourism-info}\DIFaddFL{Number of time series selected by
...
December 2017). The Base row shows the original number of series in the
structure. The numbers in parentheses show RMSSE values of base
forecasts for selected series across different levels.}}
