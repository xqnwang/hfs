...
columns indexed by the set \(\mathbb{S}\). Similarly, let
\(\bm{X}_{\mathbb{S}\cdot} \in \mathbb{R}^{|\mathbb{S}| \times c}\)
denote the submatrix of \(\bm{X}\) with the rows indexed by
\(\mathbb{S}\), where \(|\mathbb{S}|\) is the cardinality of the set
...
is no less than \(n_b\), i.e., \(|\mathbb{S}| \geq n_b\).
...
is a full rank square matrix and thus invertible. According to Theorem 2
in \citet{Zhang2023-op}, \(\bm{y}_{\mathbb{S}\cdot}\) is valid for
constructing the full hierarchy. If the solution yields a
\(\hat{\bm{G}}\) with more than \(n_b\) nonzero columns, we should be
able to find at least one subset \(\mathbb{S}^* \subset \mathbb{S}\)
with \(|\mathbb{S}^*|=n_b\) to construct an invertible square matrix
\(\bm{S}_{\mathbb{S}^{*}\cdot}\) and thus restore the full hierarchy
using a valid \(\bm{y}_{\mathbb{S}^{*}\cdot}\). Therefore, the
constraint \(\bm{GS}=\bm{I}\) ensures that the selected columns of
\(\hat{\bm{G}}\) correspond to variables that can restore the hierarchy.
\end{proof}
...
series AA and BA, but not to series AA and AB.
