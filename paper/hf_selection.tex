...
\begin{table}[!h]
\caption{\label{tbl-labour-rmse-avg}Average out-of-sample forecast results for Australian labour force data.
...
data. ``Base'' shows the average RMSE of the base forecasts, while
entries below this row indicate the percentage decrease (negative) or
increase (positive) in the average RMSE of the reconciled forecasts
compared to the base forecasts. The entries with the lowest values in
each column are highlighted in blue. In each panel, methods that
outperform the benchmark method are marked in bold. The proposed methods
are indicated with a gray background. }\tabularnewline
