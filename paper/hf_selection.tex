...
errors\todo{don't think "high forecast error" is the right term. XQ --- It should be scaled forecast errors.}.
...
this
constraint.\todo{XQ --- I've rewritten this sentence to clarify the difference between constrained and unconstrained methods.}
These methods aim to address the negative effect of some poor base
forecasts on the overall performance of the reconciled forecasts.
Additionally, through the incorporation of regularization in the
objective function, our method improves reconciliation outcomes produced
with a ``poor'' choice of \(\bm{W}\).
...
\todo{define what is N. XQ --- It should be T.} where
...
negative.\todo{Not sure of last two lines. XQ --- Has now been rewritten.}
...
estimators of \(\bm{W}\)
\todo{different estimates of W? XQ --- Corrected.} generally improve
...
WLSs-subset\todo{is it true to say STT level for WLSs-subset has RMSSE > 1, don't think so. XQ --- Has now been rewritten.}
...
errors\todo{again, "high forecast error"? XQ --- Corrected to "high scaled forecast error"}.
...
forecast
errors\todo{high forecast error? XQ --- high scaled forecast errors}
from reconciliation and enhance computational efficiency by eliminating
the need to forecast these series in future
operations\todo{don't we need forecasts for all the series to decide which columns to set to zero? XQ --- Corrected. As explained earlier, it's for future future operations.}.
...
sets\todo{are these for all hyperparameters and rolling test sets/simulations? Explain what is denoted by "-" in the table. XQ --- Corrected}.
...
forecasts\todo{prior to reconciliation or during the reconciliation, look at the equation on page 27. XQ --- It should be "during the reconciliation". Now corrected.},
