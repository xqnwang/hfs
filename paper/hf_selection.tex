...
are then aggregated higher level data. This entire process is repeated
\(500\) times.
...
as well as the whole structure (denoted as \emph{Average}). The
\emph{BU} row uses a ``bottom-up'' approach, aggregating bottom-level
base forecasts to form higher level forecasts. The rows labeled
\emph{OLS}, \emph{WLS}, \emph{WLSv}, \emph{MinT}, and \emph{MinTs}
represent the MinT method using various estimators of \(\bm{W}\) (as per
Table~\ref{tbl-bench}), while the rows \emph{OLS-}, \emph{WLS-},
\emph{WLSv-}, \emph{MinT-}, and \emph{MinTs-} denote the three methods
proposed in Section~\ref{sec-constrained}, using the corresponding
\(\bm{W}\) estimators. The \emph{EMinT} row applies the empirical MinT
method, while the \emph{Elasso} row uses the empirical group lasso
method proposed in Section~\ref{sec-unconstrained}. Notably, in each
scenario, the largest improvements occur at the level with model
