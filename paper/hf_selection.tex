...
solving Equation \eqref{eq-op_u}, is at least equal to the number of
time series at the bottom level. In addition, we can restore the full
...
the indices of nonzero columns in the solution to Equation
\eqref{eq-op_u}, \(\hat{\bm{G}}\), the following equations hold: \[
...
\textbf{Proposition 2.} \emph{The optimization problem in Equation
\eqref{eq-op_u} can be reformulated as a least squares problem with
regularization and linear equality constraint as follows:}
...
the first term in the objective of Equation \eqref{eq-lasso} as
