...
}\end{equation} for \(t=1,2,\ldots,T\), where \(T\) is the length of the
time series, and \(\bm{S}\) is an \(n \times n_b\) \emph{summing matrix}
that defines the aggregation constraints. We can write the summing
matrix as
...
\begin{proof}
Let \(\bm{X}_{\cdot \mathbb{S}} \in \mathbb{R}^{r \times c}\) denote the
matrix \(\bm{X}\) with the columns indexed by the set \(\mathbb{S}\) set
to zeros, and let
\(\bm{X}_{\mathbb{S}\cdot} \in \mathbb{R}^{r \times c}\) denote the
matrix \(\bm{X}\) with the rows indexed by \(\mathbb{S}\) set to zeros.
If \(\mathbb{S}\) is the set of indices of nonzero columns in the
solution \(\hat{\bm{G}}\) to Equation \eqref{eq-op_u}, then the
...
\] as the solution \(\hat{\bm{G}}\) is obtained by imposing
\(\bm{GS}=\bm{I}\). Therefore, the constraint \(\bm{GS}=\bm{I}\) ensures
that the selected columns of \(\hat{\bm{G}}\) correspond to variables
capable of ``restoring'' the hierarchy.
\end{proof}

For example, consider the simple hierarchy shown in
Figure~\ref{fig-hts}. Our constrained reconciliation methods might
simultaneously zero out the columns of \(\bm{G}\) corresponding to
series AA and BA, but not to series AA and AB. If the solution to
Equation \eqref{eq-op_u} yields a \(\hat{\bm{G}}\) with exactly \(n_b\)
nonzero columns, these correspond to base forecasts from which the full
hierarchy can be obtained using nothing but the information embedded in
the aggregation constraints. However, if the solution yields a
\(\hat{\bm{G}}\) with more than \(n_b\) nonzero columns, while a
coherent forecast can be restored using the the aggregation constraints,
it cannot be done uniquely due to the excess of series.
