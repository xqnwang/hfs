...
#' @export
library(ROI)
mip_l0_diag <- function(fc, S, W, G, G_bench = NULL, 
  ...
  ##                                    lambda_2 t(a) %*% a
  Q0 <- Matrix::bdiag(diag(x = 2*lambda_2, nrow = n*n),
                      matrix(0, n, n),
                      solveW) |>
    as.matrix()
  ...
  ### C3: A1 - Mz <= 0
  ### <==> kronecker(t(rep(1, n)), I_n) a - M z <= 0
  A3 <- cbind(kronecker(t(rep(1, n)), diag(x = 1, nrow = n)), 
              diag(x = -M, nrow = n), matrix(0, n, n))
  LC3 <- L_constraint(A3, rep("<=", n), rep(0, n))
  
  ### C4: A1 + M z_j >= 0 for j = 1,...,n
  ### <==> kronecker(t(rep(1, n)), I_n) a + M z >= 0
  A4 <- cbind(kronecker(t(rep(1, n)), diag(x = 1, nrow = n)), 
              diag(x = M, nrow = n), matrix(0, n, n))
  ...
  bounds(model) <- ROI::V_bound(li = c((1 + (n + 1) * seq.int(0, n-1, 1)), (n*n + n + 1):(n*n + n + n)),
                                lb = c(rep(-M, n), rep(-Inf, n)),
                                ui = seq.int(n*n),
                                ub = ifelse(seq.int(n*n) %in% (1 + (n + 1) * seq.int(0, n-1, 1)), M, 0),
