...
## For different cases of lambda_0, lambda_1, and lambda_2, the objective parameters, objective function and constraints are given respectively
...
.mip_l0 <- function(fc, S, W, G0 = NULL, 
  ...
  if(is.null(G0)){
    G0 <- stzm(nrow = NCOL(S), ncol = NROW(S))
  ...
  if(lambda_0 == 0L & lambda_1 == 0L & lambda_2 == 0L){
    ...
    Q0 <- dbind(stzm(n_b*n), solve(W))
    model <- OP(objective = Q_objective(Q = Q0))
    
    ## constraints
    ### C1: fc - kronecker(t(fc), S) %*% g <= e_check
    ### <==> kronecker(t(fc), S) %*% g + e_check >= fc
    A1 <- cbind(kronecker(t(fc), S), stdm(1, n))
    LC1 <- L_constraint(A1, eq(n), fc)
    
    ### C2: kronecker(t(S), diag(1, n_b)) %*% g = vec(diag(1, n_b))
    A2 <- cbind(kronecker(t(S), diag(1, n_b)), stzm(n_b*n_b, n))
    LC2 <- L_constraint(A2, eq(n_b*n_b), as.vector(diag(1, n_b)))
    
    constraints(model) <- rbind(LC1, LC2)

    ### types and bounds
    types(model) <- c(rep("C", n_b*n), rep("C", n))
    bounds(model) <- V_bound(li = c(1:(n_b*n), (n_b*n + 1):(n_b*n + n)), lb = rep.int(-Inf, n_b*n + n), nobj = n_parameters) # default of lower bound is 0
    ...
    n_parameters <- n_b*n + n + n_b*n
    
    # optimization problem construction
    ## Obj: 1/2 t(e_check) W^{-1} e_check + 
    ##                                    lambda_1 \sum d_positive +
    ##                                    lambda_2 t(d_positive) d_positive
    Q0 <- dbind(stzm(n_b*n), solve(W), stdm(2*lambda_2, n_b*n))
    A0 <- stm(i = rep(1, n_b*n),
              j = c((n_b*n + n + 1):(n_b*n + n + n_b*n)),
              v = rep(lambda_1, n_b*n),
    ...
    ### C1: fc - kronecker(t(fc), S) %*% g <= e_check
    ### <==> kronecker(t(fc), S) %*% g + e_check >= fc
    A1 <- cbind(kronecker(t(fc), S), stdm(1, n), stzm(n, n_b*n))
    LC1 <- L_constraint(A1, eq(n), fc)
    
    ### C2: kronecker(t(S), diag(1, n_b)) %*% g = vec(diag(1, n_b))
    A2 <- cbind(kronecker(t(S), diag(1, n_b)), stzm(n_b*n_b, n), stzm(n_b*n_b, n_b*n))
    LC2 <- L_constraint(A2, eq(n_b*n_b), as.vector(diag(1, n_b)))
    
    ### C6: g - d_positive <= vec(G0)
    A6 <- cbind(stdm(1, n_b*n), stzm(n_b*n, n), stdm(-1, n_b*n))
    LC6 <- L_constraint(A6, rep("<=", n_b*n), as.vector(G0))
    
    ### C7: g + d_positive >= vec(G0)
    A7 <- cbind(stdm(1, n_b*n), stzm(n_b*n, n), stdm(1, n_b*n))
    LC7 <- L_constraint(A7, rep(">=", n_b*n), as.vector(G0))
    
    constraints(model) <- rbind(LC1, LC2, LC6, LC7)

    ### types and bounds
    types(model) <- c(rep("C", n_b*n), rep("C", n), rep("C", n_b*n))
    bounds(model) <- V_bound(li = c(1:(n_b*n), (n_b*n + 1):(n_b*n + n)), lb = rep.int(-Inf, n_b*n + n), nobj = n_parameters) # default of lower bound is 0
    ...
    n_parameters <- n_b*n + n + n + n_b*n
    
    # optimization problem construction
    ## Obj: 1/2 t(e_check) W^{-1} e_check + lambda_0 \sum_{j=1}^{n} z_j
    Q0 <- dbind(stzm(n_b*n), solve(W), stzm(n), stzm(n_b*n))
    A0 <- stm(i = rep(1, n),
              j = c((n_b*n + n + 1):(n_b*n + n + n)),
              v = c(rep(lambda_0, n)),
              nrow = 1, ncol = n_parameters)
    model <- OP(objective = Q_objective(Q = Q0, L = A0))
    ...
    ### C1: fc - kronecker(t(fc), S) %*% g <= e_check
    ### <==> kronecker(t(fc), S) %*% g + e_check >= fc
    A1 <- cbind(kronecker(t(fc), S), stdm(1, n), stzm(n), stzm(n, n_b*n))
    LC1 <- L_constraint(A1, eq(n), fc)
    
    ### C2: kronecker(t(S), diag(1, n_b)) %*% g = vec(diag(1, n_b))
    A2 <- cbind(kronecker(t(S), diag(1, n_b)), stzm(n_b*n_b, n), stzm(n_b*n_b, n), stzm(n_b*n_b, n_b*n))
    LC2 <- L_constraint(A2, eq(n_b*n_b), as.vector(diag(1, n_b)))
    
    ### C3: \sum g_positive_{(1+(j-1)*n_b):(j*n_b)} - M z_j <= 0 for j = 1,...,n
    A3 <- sapply(1:n, function(j){
      L_j <- stm(i = rep(1, 1 + n_b),
                 j = c(n_b*n + n + j, n_b*n + n + n + (1+(j-1)*n_b):(j*n_b)),
                 v = c(-M, rep(1, n_b)),
                 nrow = 1, ncol = n_parameters)
      as.matrix(L_j)
    })  %>% t()
    ...
    ### C4: g - g_positive <= 0
    A4 <- cbind(stdm(1, n_b*n), stzm(n_b*n, n), stzm(n_b*n, n), stdm(-1, n_b*n))
    LC4 <- L_constraint(A4, rep("<=", n_b*n), rep(0, n_b*n))
    
    ### C5: g + g_positive >= 0
    A5 <- cbind(stdm(1, n_b*n), stzm(n_b*n, n), stzm(n_b*n, n), stdm(1, n_b*n))
    LC5 <- L_constraint(A5, rep(">=", n_b*n), rep(0, n_b*n))
    
    constraints(model) <- rbind(LC1, LC2, LC3, LC4, LC5)

    ### z_g \in {0, 1}
    types(model) <- c(rep("C", n_b*n), rep("C", n), rep("B", n), 
                      rep("C", n_b*n))
    bounds(model) <- V_bound(li = c(1:(n_b*n), (n_b*n + 1):(n_b*n + n)), lb = rep.int(-Inf, n_b*n + n), nobj = n_parameters) # default of lower bound is 0
    ...
    ##                                    lambda_1 \sum d_positive +
    ##                                    lambda_2 t(d_positive) d_positive
    Q0 <- dbind(stzm(n_b*n), solve(W), stzm(n), stzm(n_b*n), stdm(2*lambda_2, n_b*n))
    A0 <- stm(i = rep(1, n + n_b*n),
              j = c((n_b*n + n + 1):(n_b*n + n + n), (n_b*n + n + n + n_b*n + 1):(n_b*n + n + n + n_b*n + n_b*n)),
    ...
    ### C1: fc - kronecker(t(fc), S) %*% g <= e_check
    ### <==> kronecker(t(fc), S) %*% g + e_check >= fc
    A1 <- cbind(kronecker(t(fc), S), stdm(1, n), stzm(n), stzm(n, n_b*n), stzm(n, n_b*n))
    LC1 <- L_constraint(A1, eq(n), fc)
    ...
    ### C4: g - g_positive <= 0
    A4 <- cbind(stdm(1, n_b*n), stzm(n_b*n, n), stzm(n_b*n, n), stdm(-1, n_b*n), stzm(n_b*n))
    LC4 <- L_constraint(A4, rep("<=", n_b*n), rep(0, n_b*n))
    
    ### C5: g + g_positive >= 0
    A5 <- cbind(stdm(1, n_b*n), stzm(n_b*n, n), stzm(n_b*n, n), stdm(1, n_b*n), stzm(n_b*n))
    LC5 <- L_constraint(A5, rep(">=", n_b*n), rep(0, n_b*n))
    
    ### C6: g - d_positive <= vec(G0)
    A6 <- cbind(stdm(1, n_b*n), stzm(n_b*n, n), stzm(n_b*n, n), stzm(n_b*n), stdm(-1, n_b*n))
    LC6 <- L_constraint(A6, rep("<=", n_b*n), as.vector(G0))
    
    ### C7: g + d_positive >= vec(G0)
    A7 <- cbind(stdm(1, n_b*n), stzm(n_b*n, n), stzm(n_b*n, n), stzm(n_b*n), stdm(1, n_b*n))
    LC7 <- L_constraint(A7, rep(">=", n_b*n), as.vector(G0))
